...
def read_measurements_from_csv(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for row in reader:
            x = float(row[7])  # Example: X coordinate column
            y = float(row[8])  # Example: Y coordinate column
            z = float(row[9])  # Example: Z coordinate column
            mt = float(row[10])  # Example: Time column
...
def main():
    ...
    # Initialize targets for JPDA (example targets, adjust as needed)
            ...
            if not kalman_filter.first_rep_flag:
                ...
                if clusters:
                    ...
                    if hypotheses:
                        ...
                        r_val, az_val, el_val = cart2sph(best_hypothesis[0][0], best_hypothesis[0][1], best_hypothesis[0][2])
                        r.append(r_val)
                        az.append(az_val)
                        el.append(el_val)
